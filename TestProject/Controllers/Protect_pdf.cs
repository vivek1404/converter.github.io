...
namespace TestProject.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class protect_pdf : ControllerBase
    {
        ...
        public async Task<IActionResult> convert_protect_pdf([FromForm] Protectmodel model)
        {
            try
            {
                ...
                var task = lovePdfAPi.CreateTask<ProtectTask>();
                FileRecord file = await SaveFileAsync(model.MyFile);
                ...
                ProtectParams test = new ProtectParams(model.Confirm_password);
                //var files = task.AddFile(model.MyFile);
                var time = task.Process(test);
                //task.DownloadFile("C:\\Users\\vivek.kumar2\\source\\repos\\Practice");
                string pathToDownload = "C:\\Users\\vivek.kumar2\\Downloads\\output.pdf";
