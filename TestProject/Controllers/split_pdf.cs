...
namespace TestProject.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class split_pdf : ControllerBase
    {
        ...
        public async Task<IActionResult> convert_split_pdf([FromForm] FileModel model)
        {
            try
            {
                ...
                var task = lovePdfAPi.CreateTask<SplitTask>();
                FileRecord file = await SaveFileAsync(model.MyFile);
                ...
                var time = task.Process(new SplitParams(new SplitModeRanges("2-4,6-8"))
                {
                    MergeAfter = true
                });
