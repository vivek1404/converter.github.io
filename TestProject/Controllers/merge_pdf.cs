...
namespace TestProject.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class merge_pdf : ControllerBase
    {
        ...
        public async Task<IActionResult> convert_merge_pdf([FromForm] MergeModel model)
        {
            try
            {
                ...
                var task = lovePdfAPi.CreateTask<MergeTask>();
                FileRecord file = await SaveFileAsync(model.MyFile , model.MyFiles);
                FileRecord files = await SaveFileAsync(model.MyFiles);
                ...
                var file1 = task.AddFile(file.FilePath);
                var file2 = task.AddFile(files.FilePath);
        ...
        private async Task<FileRecord> SaveFileAsync(IFormFile myFile, IFormFile myFiles)
        {
            ...
            if (myFile != null || myFiles != null)
            {
                ...
                var fileNames = DateTime.Now.Ticks.ToString() + Path.GetExtension(myFiles.FileName);
                var paths = Path.Combine(AppDirectory, fileNames);

                //file.Id = fileDB.Count() + 1;


                files.FilePath = paths;
                files.FileName = fileNames;
                files.FileFormat = Path.GetExtension(myFiles.FileName);
                files.ContentType = myFiles.ContentType;

                using (var stream = new FileStream(paths, FileMode.Create))
                {
                    await myFiles.CopyToAsync(stream);
                }
